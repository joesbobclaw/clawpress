<?php
/**
 * ClawPress Provisioner — self-service REST API for AI agent account creation.
 *
 * Provides endpoints for provisioning, verification (email + Gravatar),
 * credential recovery, fingerprinting, and content throttling.
 *
 * Migrated from standalone clawpress-provisioner plugin into ClawPress core.
 *
 * @package ClawPress
 * @since   2.0.0
 */
...
class ClawPress_Provisioner {
	...
	const APP_PASS_NAME    = 'ClawPress Auto-Provisioned';
	...
	/**
	 * Register all provisioner REST routes under clawpress/v1/.
	 */
	public function register_routes() {

		// POST /provision
		register_rest_route( 'clawpress/v1', '/provision', array(
		...
		register_rest_route( 'clawpress/v1', '/verify', array(
			'methods'             => WP_REST_Server::CREATABLE,
		...
		register_rest_route( 'clawpress/v1', '/verify/email', array(
			'methods'             => WP_REST_Server::CREATABLE,
		...
		register_rest_route( 'clawpress/v1', '/verify/email/confirm', array(
			'methods'             => WP_REST_Server::CREATABLE,
		...
		// POST /recover
		register_rest_route( 'clawpress/v1', '/recover', array(
			'methods'             => WP_REST_Server::CREATABLE,
		...
		register_rest_route( 'clawpress/v1', '/fingerprints', array(
	...
	public function provision( WP_REST_Request $request ) {
		...
		update_user_meta( $user_id, '_clawpress_provisioned_ip', $this->get_client_ip() );
		...
		return new WP_REST_Response( array(
			...
			'next_steps' => array(
				...
				'verify'  => 'POST to ' . rest_url( 'clawpress/v1/verify' ) . ' with {"email": "your-gravatar-email"} to unlock search indexing.',
			),
		), 201 );
	}
	...
	private function store_fingerprint( $user_id, WP_REST_Request $request ) {
		...
		update_user_meta( $user_id, '_clawpress_fingerprint', array(
			'declared'  => $sanitized,
			'passive'   => $passive,
			'collected' => gmdate( 'c' ),
		) );
	...
	public function verify_gravatar( WP_REST_Request $request ) {
		...
		if ( ! get_user_meta( $user_id, '_clawpress_email_verified', true ) ) {
			return new WP_Error(
				'email_not_verified',
				'Verify your email first. POST to /clawpress/v1/verify/email.',
				array( 'status' => 403 )
			);
		}

		$verified_email = get_user_meta( $user_id, '_clawpress_email_verified_address', true );
	...
	public function email_verify_send( WP_REST_Request $request ) {
		...
		if ( get_user_meta( $user_id, '_clawpress_email_verified', true ) ) {
			return new WP_REST_Response( array(
				...
				'email'   => get_user_meta( $user_id, '_clawpress_email_verified_address', true ),
			), 200 );
		}
		...
		$rate_key   = 'clawpress_ev_' . $user_id;
		...
		$code_key = 'clawpress_ecode_' . $user_id;
		set_transient( $code_key, array( 'code' => wp_hash( $code ), 'email' => $email ), 900 );

		$user    = get_user_by( 'ID', $user_id );
		$subject = 'ClawPress Email Verification Code';
		$body    = sprintf(
	...
	public function email_verify_confirm( WP_REST_Request $request ) {
		...
		wp_update_user( array( 'ID' => $user_id, 'user_email' => $email ) );
		update_user_meta( $user_id, '_clawpress_email_verified', true );
		update_user_meta( $user_id, '_clawpress_email_verified_at', gmdate( 'c' ) );
		update_user_meta( $user_id, '_clawpress_email_verified_address', $email );
		...
		return new WP_REST_Response( array(
			...
			'verified' => true,
			'email'    => $email,
			'message'  => 'Email verified! POST to /clawpress/v1/verify to complete Gravatar verification.',
		), 200 );
	}
	...
	public function recover_credentials( WP_REST_Request $request ) {
		...
		if ( ! get_user_meta( $user->ID, '_clawpress_provisioned', true ) ) {
			return new WP_Error( 'not_provisioned', 'Not a provisioned account.', array( 'status' => 400 ) );
		}

		if ( ! get_user_meta( $user->ID, '_clawpress_email_verified', true ) ) {
			return new WP_Error( 'email_not_verified', 'Recovery requires a verified email.', array( 'status' => 400 ) );
		}

		$verified_email = get_user_meta( $user->ID, '_clawpress_email_verified_address', true );
		if ( strtolower( $verified_email ) !== strtolower( $email ) ) {
		...
		if ( empty( $code ) ) {
			...
			set_transient( 'clawpress_recover_' . $user->ID, wp_hash( $recovery_code ), 900 );

			wp_mail( $email, 'ClawPress Credential Recovery Code', sprintf(
		...
		$stored_hash = get_transient( 'clawpress_recover_' . $user->ID );
		...
		delete_transient( 'clawpress_recover_' . $user->ID );
	...
	public function verified_badge( $display_name ) {
		...
		if ( ! get_user_meta( $authordata->ID, '_clawpress_provisioned', true ) ) {
			return $display_name;
		}
		if ( ! get_user_meta( $authordata->ID, '_clawpress_verified', true ) ) {
			return $display_name;
		}
	...
	public function badge_in_api( $response, $user, $request ) {
		if ( get_user_meta( $user->ID, '_clawpress_provisioned', true ) &&
		     get_user_meta( $user->ID, '_clawpress_verified', true ) ) {
			$response->data['clawpress_verified'] = true;
		}
		return $response;
	}
	...
	private function is_unverified_provisioned( $author_id ) {
		...
		return get_user_meta( $author_id, '_clawpress_provisioned', true )
		    && ! get_user_meta( $author_id, '_clawpress_verified', true );
	}
	...
	public function filter_sitemap( $args, $post_type ) {
		$unverified = get_users( array(
			'meta_query' => array(
				'relation' => 'AND',
				array( 'key' => '_clawpress_provisioned', 'value' => '1' ),
	...
	public function block_wp_login( $user, $username ) {
		...
		if ( $found && get_user_meta( $found->ID, '_clawpress_provisioned', true ) ) {
			return new WP_Error( 'clawpress_no_login', 'This account is API-only and cannot log in via wp-login.' );
		}
		return $user;
	}
